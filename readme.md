![Status](https://img.shields.io/badge/CMD_Status_|_-Working-404040) 
![Online](https://img.shields.io/badge/Script_Status_|_-Working-white)   
![Version](https://img.shields.io/badge/CMD_Version_|_-v1.0.1-404040) 
![Version](https://img.shields.io/badge/Script_Version_|_-v2.1.55-404040)
 
#### **Hello,it's DxBreak Team**
#### Join to discord !
###

## DxBreak [Live | v2.1.55]
```lua
loadstring(game:HttpGet("wehface.vercel.app/loader"))()
```
## Command Bar [v1.0.1]
```lua
loadstring(game:HttpGet("wehface.vercel.app/cb"))()
```
## Need more?
### Site    | [Click](dxbreak.vercel.app)
### Hosting | [Click](wehface.vercel.app)
### Discord | [Click](discord.gg/waVwyMJdm8)

## Other
#### Old Examples of "xenapsu" [v2.5.54]
```lua
loadstring(game:HttpGet("https://weh-face.vercel.app/old-DXBRE"))()
```
##                                                          Special thank's                
ovasse & vellyr 

### Credit's
#### UI - WindUI  | Idea - ovasse⠀⠀&⠀⠀UI - Cerberus | Rewrited - baupexㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ
#### CMD - ovasse | Idea - ovasse & baupex
#### Site - ovasse & vellyr | Idea - ovasse & vellyr & baupex⠀
##### ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀ ⠀⠀   DxBreak Team © 2025 - 2026
![dxbre banner](https://raw.githubusercontent.com/lixeal/DXBRE/main/res/dxbre_corner-banner.png)
